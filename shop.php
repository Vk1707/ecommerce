...
if (isset($_GET['category_id'])) {
    $id = $_GET['category_id'];
    $prod_sql = "SELECT * FROM product WHERE category_id = $id ORDER BY id DESC LIMIT $limit OFFSET $offset";
    $products = $db->select($prod_sql);

    // Get the total count for pagination
    $total_products = $db->select_single("SELECT COUNT(*) as total FROM product WHERE category_id = $id");

    $sql = "SELECT name FROM product_category WHERE id = $id";
    $categoryName = $db->select_single($sql);
} else {
    $prod_sql = "SELECT * FROM product ORDER BY id DESC LIMIT $limit OFFSET $offset";
    $products = $db->select($prod_sql);

    // Get the total count for pagination
    $total_products = $db->select_single("SELECT COUNT(*) as total FROM product");
}
...
<head>
    <meta charset="utf-8">
    <title>Hot Shopping Deals</title>
...
?>
            ...
            <div class="col-12">
                <nav class="breadcrumb bg-light mb-30">
                    ...
                    <?php if(isset($_GET['category_id'])){ ?>
                        <span class='breadcrumb-item active'><?= $categoryName['name'] ?></span>
                        <span class='breadcrumb-item active'>All Products</span>
                    <?php } else{
                         echo "<a class='breadcrumb-item text-dark' href='shop.php'>Shop</a>";
                        echo "<span class='breadcrumb-item active'>All Products</span>";
                    }
                    ?>
                </nav>
            </div>
    ...
    <div class="container-fluid">
        <?php if(isset($_GET['category_id'])){ ?>
           <h2 class="section-title position-relative text-uppercase mx-xl-5 mb-4"><span class="bg-secondary pr-3"><?= $categoryName['name'] ?> Products</span></h2>
       <?php } ?>
        <div class="row px-xl-5">
            <!-- Shop Product Start -->
    ...
    <!-- Pagination For Cattegory Product -->
    <?php if(isset($_GET['category_id'])): ?>
        <div class="col-12">
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                ...
                <?php if ($page > 1): ?>
                    <li class="page-item">
                        <a class="page-link" href="?category_id=<?= $id ?>&page=<?= $page - 1 ?>" aria-label="Previous">&laquo;</a>
                    </li>
                <?php endif; ?>
                ...
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                        <a class="page-link" href="?category_id=<?= $id ?>&page=<?= $i ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
                ...
                <?php if ($page < $total_pages): ?>
                    <li class="page-item">
                        <a class="page-link" href="?category_id=<?= $id ?>&page=<?= $page + 1 ?>">&raquo;</a>
                    </li>
                <?php endif; ?>
            </ul>
        </nav>
