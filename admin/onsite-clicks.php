...
SELECT 
        ...
        oc.click_date,
        p.name AS prod_name,
        p.sale_price AS prod_price,
        u.name AS user_name
    FROM 
        onsite_clicks oc
    LEFT JOIN 
        product p ON oc.prod_id = p.id
    LEFT JOIN 
        user u ON oc.user_id = u.user_id
    ORDER BY 
        oc.click_date DESC
";
...
<div class="layout-navbar container-xxl navbar navbar-expand-xl navbar-detached align-items-center bg-navbar-theme">
  <div class="d-flex col-lg-12 justify-content-between">
      <div class="fw-bold h4 m-auto">Onsite Clicks</div>
    </div>
</div>
<div class="container-xxl flex-grow-1 container-p-y">
    <table id="click-list" class="table">
        <thead>
            <tr>
                <th style="text-align: left;">Id</th>
                <th style="text-align: center;" width="10%">P Id</th>
                <th>Product</th>
                <th>Price</th>
                <th>U Id</th>
                <th>User</th>
        ...
        <tbody class="table-border-bottom-0">
            ...
            <tr>
                ...
                <td style="text-align: center;"><?= $click['prod_id']; ?></td>
                <td><?= $click['prod_name']; ?></td>
                <td><?= $click['prod_price']; ?></td>
                <td><?= $click['user_id'] ?></td>
                <td><?= empty($click['user_name'])?'Guest':$click['user_name'] ?></td>
                <td><?= !empty($click['session_id'])?substr($click['session_id'],0,15)."...":"" ?></td>
                <td><?= $click['click_date']; ?></td>
            </tr>
            <?php
            }
            ?>
