              ...
              <div class="table-responsive p-0">
                <table class="table align-items-left mb-0">
                  <thead>
                    ...
                    <th class=""><b>Unique Visitors</b></td> 
                   
                    </tr>
                  </thead>
                  <?php
        $i=1;
        $sql="SELECT date(update_time) as vdate,count(*) as visits FROM visitors group BY date(update_time) order by date(update_time) desc";

        $rs=$db->select($sql);
        foreach ($rs as $row) 
          {
            $vdate=$row['vdate'];
            $link="website-visitors-datewise.php?vd=$vdate";
          ?>
                  <tbody>
          <tr>
           ...
           <td class="text-left text-sm">
            <p class="text-xs font-weight-bold mb-0"><?php echo $row['vdate'];?></p>
          </td>
           <td class="text-left text-sm">
            <p class="text-xs font-weight-bold mb-0"><a href=<?php echo $link;?>><?php echo $row['visits'];?></a></p>
          </td>
